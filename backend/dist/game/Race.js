...
export class Race {
    id;
    trackLength = 4000;
    raceDurationMs = 180000;
    cars = [];
    status = 'waiting';
    startTime = 0;
    startCountdownAt = 0;
    bettingPool = {}; // carId -> bets
    bettorNames = {}; // userId -> bettor name
    constructor(id) {
        this.id = id;
        // spawn NPCs
        for (let i = 0; i < 3; i++) {
    ...
    placeBet(userId, amount, carId, bettorName) {
        if (!this.bettingPool[carId]) {
            this.bettingPool[carId] = [];
        }
        const bet = { userId, amount, carId };
    ...
    useAbility(userId, ability, targetId) {
        const car = this.cars.find(c => c.ownerId === userId);
        if (!car)
            return { success: false, message: 'You are not driving a racer.' };
        if (this.status !== 'active')
            return { success: false, message: 'Abilities can only be used during an active race.' };
        if (ability === 'nitro') {
            if (car.nitroBoostRemaining > 0)
                return { success: false, message: 'Nitro Boost is already active.' };
            car.nitroUsed = true;
        ...
        if (ability === 'risk') {
            if (car.riskShaftRemaining > 0)
                return { success: false, message: 'Risk Shaft is already active.' };
            car.riskUsed = true;
            car.riskShaftRemaining = 15000;
            return { success: true, message: 'Risk Shaft is now being used, your speed is now 3x for 15 seconds!' };
        }
        if (ability === 'shield') {
            if (car.shieldRemaining > 0)
                return { success: false, message: 'Safety Shield is already active.' };
            car.shieldUsed = true;
            car.shieldRemaining = 30000;
            return { success: true, message: 'Safety Shield is now being used, you are protected for 30 seconds!' };
        }
        if (ability === 'collide') {
            if (car.speedPenaltyRemaining > 0)
                return { success: false, message: 'Wait for the collide penalty to end before using it again.' };
